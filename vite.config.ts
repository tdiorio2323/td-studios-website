...
const handleListPublic = (req: IncomingMessage, res: ServerResponse) => {
  if (req.method !== 'GET') {
    res.statusCode = 405;
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      if (!abs.startsWith(publicRoot) || !fs.existsSync(abs) || !fs.statSync(abs).isDirectory()) {
        out[dir] = [];
        continue;
      }
...
const handleScaffold = (req: IncomingMessage, res: ServerResponse) => {
  if (req.method !== 'POST') {
    res.statusCode = 405;
    res.end('Method Not Allowed');
    return;
...
const devPlugins: Plugin[] = [
  {
    ...
    configureServer(server) {
      server.middlewares.use('/__list-public', handleListPublic);
      server.middlewares.use('/__scaffold', handleScaffold);
    }
  }
];
